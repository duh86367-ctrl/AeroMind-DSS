...
def load_airspace_data():
    data = {
        ...
        'vertical_rate': [-15.0, 5.0, -8.0, 0.0, 0.0],       # m/s
...
with col_left:
    st.subheader("🛰️ 실시간 전술 공역 레이더 (고화질 위성 지도)")
    
    # 오픈소스 고화질 위성지도 타일 레이어 추가 (토큰 불필요)
    ...
    aircraft_layer = pdk.Layer(
        ...
        get_color="[color_r, color_g, 50, 230]",
        get_radius=700,
        pickable=True,
    )
    
    # 레이블 텍스트 레이어
    df['label'] = df.apply(lambda r: f"{r['callsign']}\n{r['geo_altitude']}ft\n{r['velocity']}kts", axis=1)
    ...
    # 맵 스타일을 완전히 비우고(custom) 타일 레이어를 수동 배치하여 위성 이미지 강제 활성화
    st.pydeck_chart(pdk.Deck(
        layers=[satellite_tile_layer, aircraft_layer, text_layer],
...
with col_right:
    ...
    for idx, row in df_queue.iterrows():
        box_style = "🔴 CRITICAL" if row['risk_score'] > 70 else "🟡 WARNING" if row['risk_score'] > 40 else "🟢 STABLE"
